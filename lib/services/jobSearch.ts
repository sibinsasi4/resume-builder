...
import { fetchRemoteOkJobs } from '../scraper/remoteOk';
import { fetchWWRJobs } from '../scraper/weworkremotely';
import { fetchJoobleJobs } from '../scraper/jooble';
import { scrapeGoogleJobs } from '../scraper/googleJobs';
...
export async function searchJobs(query: string, location: string = ''): Promise<Job[]> {
    try {
        ...
        const [joobleJobs, remoteOkJobs, wwrJobs, linkedInJobs] = await Promise.all([
            fetchJoobleJobs(query, location),
        ...
        if (allJobs.length > 0) {
            ...
            const [genericJooble, genericRemote, genericWWR] = await Promise.all([
                fetchJoobleJobs('developer', location),
