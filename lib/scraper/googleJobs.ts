...
export interface ScrapedJob {
    ...
    location: string;
    postedAt: string;
    platform: 'Google Jobs' | 'SimplyHired' | 'RemoteOK' | 'WeWorkRemotely' | 'Jooble' | 'Other';
    applyUrl: string;
    description: string;
    salary?: string;
}

export async function scrapeGoogleJobs(query: string, location: string): Promise<ScrapedJob[]> {
    let browser;
    try {
