...
import { ScrapedJob } from './googleJobs'; // Reuse interface

export async function scrapeSimplyHired(query: string, location: string): Promise<ScrapedJob[]> {
    let browser;
    try {
        ...
        const jobs = await page.evaluate(() => {
            ...
            cards.forEach((card) => {
                try {
                    ...
                    let applyUrl = (titleEl as HTMLAnchorElement)?.href || '';
...
function getMockJobs(query: string, location: string): ScrapedJob[] {
    return [
        {
